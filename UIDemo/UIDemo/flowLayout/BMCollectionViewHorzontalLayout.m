//
//  BMCollectionViewHorzontalLayout.m
//  UIDemo
//
...
@interface BMCollectionViewHorzontalLayout()

@end

@implementation BMCollectionViewHorzontalLayout
    ...
    for (int index= 0; index<sectionCount; index++) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(collectionView:layout:registerBackView:)]) {
            ...
            if (className != nil && className.length > 0) {
                ...
                [self registerClass:[BMCollectionReusableView class] forDecorationViewOfKind:@"BMCollectionReusableView"];
            }
        }
        else {
            [self registerClass:[BMCollectionReusableView class] forDecorationViewOfKind:@"BMCollectionReusableView"];
        ...
        if (headerW > 0) {
            ...
            BMCollectionViewLayoutAttributes* headerAttr = [BMCollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader withIndexPath:headerIndexPath];
        ...
        if (itemCount > 0) {
            ...
            NSAssert((self.layoutType==BMLayoutTypeLabelVerticalLayout||self.layoutType==BMLayoutTypeColumnLayout||self.layoutType==BMLayoutTypeAbsoluteLayout), @"横向布局暂时只支持LabelVerticalLayout,ColumnLayout,AbsoluteLayout!");
            ...
            for (int i=0; i<itemCount; i++) {
                ...
                BMCollectionViewLayoutAttributes *attributes = [BMCollectionViewLayoutAttributes layoutAttributesForCellWithIndexPath:indexPath];
                
                NSInteger preRow = self.attributesArray.count - 1;
                switch (self.layoutType) {
#pragma mark 纵向标签布局处理
                    case BMLayoutTypeLabelVerticalLayout: {
                        //找上一个cell
                        if(preRow >= 0){
                            if(i > 0) {
                                BMCollectionViewLayoutAttributes *preAttr = self.attributesArray[preRow];
                        ...
                        break;
                    case BMLayoutTypeLabelHorizontalLayout: {
                        
                    }
                        break;
#pragma mark 列布局处理 | 横向标签布局处理
                    case BMLayoutTypeColumnLayout: {
                        CGFloat max = CGFLOAT_MAX;
                        NSInteger column = 0;
                        if (self.columnSortType == BMColumnSortTypeSequence) {
                        ...
                        break;
                    case BMLayoutTypeAbsoluteLayout: {
                ...
                if (self.layoutType != BMLayoutTypePercentLayout) {
                ...
                if (self.layoutType == BMLayoutTypeColumnLayout) {
                    ...
                    lastX = max;
                } else if (self.layoutType == BMLayoutTypeAbsoluteLayout) {
                    if (i==itemCount-1) {
                        for (BMCollectionViewLayoutAttributes* attr in arrayOfAbsolute) {
        ...
        if (footerW > 0) {
            ...
            BMCollectionViewLayoutAttributes *footerAttr = [BMCollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionFooter withIndexPath:footerIndexPath];
...
#pragma mark 添加背景视图
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(collectionView:layout:registerBackView:)]) {
            NSString* className = [self.delegate collectionView:self.collectionView layout:self registerBackView:index];
            if (className != nil && className.length > 0) {
                BMCollectionViewBackgroundViewLayoutAttributes *attr = [BMCollectionViewBackgroundViewLayoutAttributes  layoutAttributesForDecorationViewOfKind:className withIndexPath:[NSIndexPath indexPathForRow:0 inSection:index]];
                ...
                BMCollectionViewLayoutAttributes *attr = [BMCollectionViewLayoutAttributes  layoutAttributesForDecorationViewOfKind:@"BMCollectionReusableView" withIndexPath:[NSIndexPath indexPathForRow:0 inSection:index]];
                attr.frame = CGRectMake([self isAttachToTop:index]?itemStartX-headerW:itemStartX, 0, backWidth, self.collectionView.frame.size.height);
...
//        BMCollectionViewLayoutAttributes* attr = self.attributesArray[i];
