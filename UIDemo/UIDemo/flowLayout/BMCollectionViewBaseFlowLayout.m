//
//  BMCollectionViewBaseFlowLayout.m
//  UIDemo
//
...
#import "BMCellFakeView.h"

typedef NS_ENUM(NSUInteger, BMScrollDirction) {
    BMScrollDirctionStay,
    BMScrollDirctionToTop,
    BMScrollDirctionToEnd,
};

@interface BMCollectionViewBaseFlowLayout ()
<UIGestureRecognizerDelegate>

//关于拖动的参数
@property (nonatomic, strong) BMCellFakeView *cellFakeView;
@property (nonatomic, strong) UILongPressGestureRecognizer *longPress;
@property (nonatomic, strong) UIPanGestureRecognizer *panGesture;
@property (nonatomic, assign) CGPoint fakeCellCenter;
@property (nonatomic, assign) CGPoint panTranslation;
@property (nonatomic, assign) BMScrollDirction continuousScrollDirection;
@property (nonatomic, strong) CADisplayLink *displayLink;

@end

@implementation BMCollectionViewBaseFlowLayout
{
...
- (instancetype)init {
    if (self == [super init]) {
        ...
        self.layoutType = BMLayoutTypeFillLayout;
        self.columnCount = 1;
        self.columnSortType = BMColumnSortTypeMinHeight;
        self.fixTop = 0;
        self.xBeyond = 3;
...
//    return [BMCollectionViewLayoutAttributes class];
//}
            ...
            for (UICollectionViewLayoutAttributes *attriture in self.headerAttributesArray) {
                ...
                if (!isNeedChangeFrame) {
                    /*
                     ...
                     否则header无法正确归位
                     */
                    if ([attriture isKindOfClass:[BMCollectionViewLayoutAttributes class]]) {
                        attriture.frame = ((BMCollectionViewLayoutAttributes*)attriture).orginalFrame;
                    }
                }
            }
        }
...
// 暂时去掉layoutAttributesForItemAtIndexPath方法
...
- (void)setUpGestureRecognizers
{
    ...
    __weak typeof(BMCollectionViewBaseFlowLayout*) weakSelf = self;
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan:{
            ...
            self.cellFakeView = [[BMCellFakeView alloc]initWithCell:currentCell];
...
- (void)beginScrollIfNeeded
{
    ...
    if(fakeCellTopEdge <= offset + paddingTop + trigerInsetTop){
        self.continuousScrollDirection = BMScrollDirctionToTop;
        [self setUpDisplayLink];
    }else if(fakeCellEndEdge >= offset + length - paddingEnd - trigerInsetEnd) {
        self.continuousScrollDirection = BMScrollDirctionToEnd;
        [self setUpDisplayLink];
    }else {
        [self invalidateDisplayLink];
    }
...
- (void)moveItemIfNeeded {
    ...
    for (BMCollectionViewLayoutAttributes* attr in weakSelf.attributesArray) {
...
{
    _continuousScrollDirection = BMScrollDirctionStay;
...
- (CGFloat)calcTrigerPercentage
{
    ...
    if (self.continuousScrollDirection == BMScrollDirctionToTop) {
        if (self.cellFakeView) {
    ...
    } else if (self.continuousScrollDirection == BMScrollDirctionToEnd){
        if (self.cellFakeView) {
...
- (CGFloat)scrollValueWithSpeed:(CGFloat)speed andPercentage:(CGFloat)percentage
{
    ...
    switch (_continuousScrollDirection) {
        case BMScrollDirctionStay: {
            return 0.0f;
            break;
        }
        case BMScrollDirctionToTop: {
            value = -speed;
            break;
        }
        case BMScrollDirctionToEnd: {
            value = speed;
            break;
        }
        default: {
