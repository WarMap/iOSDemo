//
//  BMCollectionViewBaseFlowLayout.h
//  UIDemo
//
...
typedef NS_ENUM(NSUInteger, BMLayoutType)
{
    BMLayoutTypeLabelHorizontalLayout   = 1,  //标签横向
    BMLayoutTypeLabelVerticalLayout     = 2,  //标签纵向
    BMLayoutTypeColumnLayout            = 3,  //列布局    指定列数，按列数来等分一整行，itemSize的width可以任意写，
                                                        //在布局中会自动帮你计算。可用于瀑布流，普通UITableViewCell
    BMLayoutTypePercentLayout           = 4,  //百分比布局     需实现percentOfRow的代理，
                                                            //根据设定值来计算每个itemSize的宽度
    BMLayoutTypeFillLayout              = 5,  //填充式布局     将一堆大小不一的view见缝插针的填充到一个平面内，
                                                            //规则为先判断从左到右是否有间隙填充，再从上到下判断。
    BMLayoutTypeAbsoluteLayout          = 6,  //绝对定位布局    需实现rectOfItem的代理，指定每个item的frame
};

typedef NS_ENUM(NSUInteger, BMColumnSortType)
{
    BMColumnSortTypeMinHeight               = 1,        // 按最小高度
    BMColumnSortTypeSequence                = 2,        // 按顺序
};

@class BMCollectionViewBaseFlowLayout;

@protocol  BMCollectionViewBaseFlowLayoutDelegate <NSObject, UICollectionViewDelegateFlowLayout>
...
//设置每个section的背景图
- (UIImage*)collectionView:(UICollectionView *)collectionView
                    layout:(UICollectionViewFlowLayout *)collectionViewLayout
       backImageForSection:(NSInteger)section;

//自定义每个section的背景view，需要继承UICollectionReusableView(如要调用方法传递参数需要继承BMCollectionBaseDecorationView)，返回类名
- (NSString*)collectionView:(UICollectionView *)collectionView
                     layout:(UICollectionViewFlowLayout *)collectionViewLayout
           registerBackView:(NSInteger)section;

//向每个section自定义背景view传递自定义方法 eventName:方法名（注意带参数的方法名必须末尾加:）,parameter:参数
- (BMBaseEventModel*)collectionView:(UICollectionView *)collectionView
                             layout:(UICollectionViewFlowLayout *)collectionViewLayout
     backgroundViewMethodForSection:(NSInteger)section;

//背景是否延伸覆盖到headerView，默认为NO
- (BOOL)collectionView:(UICollectionView *)collectionView
                layout:(UICollectionViewFlowLayout *)collectionViewLayout
           attachToTop:(NSInteger)section;

//背景是否延伸覆盖到footerView，默认为NO
- (BOOL)collectionView:(UICollectionView *)collectionView
                layout:(UICollectionViewFlowLayout *)collectionViewLayout
        attachToBottom:(NSInteger)section;

/******** 提取出UICollectionViewLayoutAttributes的一些属性 ***********/
//设置每个item的zIndex，不指定默认为0
- (NSInteger)collectionView:(UICollectionView *)collectionView
                     layout:(UICollectionViewFlowLayout *)collectionViewLayout
               zIndexOfItem:(NSIndexPath*)indexPath;

//设置每个item的CATransform3D，不指定默认为CATransform3DIdentity
- (CATransform3D)collectionView:(UICollectionView *)collectionView
                         layout:(UICollectionViewFlowLayout *)collectionViewLayout
                transformOfItem:(NSIndexPath*)indexPath;

//设置每个item的alpha，不指定默认为1
- (CGFloat)collectionView:(UICollectionView *)collectionView
                   layout:(UICollectionViewFlowLayout *)collectionViewLayout
              alphaOfItem:(NSIndexPath*)indexPath;

/******** ClosedLayout列布局需要的代理 ***********/
//在ClosedLayout列布局中指定一行有几列，不指定默认为1列
- (NSInteger)collectionView:(UICollectionView *)collectionView
                     layout:(UICollectionViewFlowLayout*)collectionViewLayout
       columnCountOfSection:(NSInteger)section;
...
- (CGFloat)collectionView:(UICollectionView *)collectionView
                   layout:(UICollectionViewFlowLayout*)collectionViewLayout
             percentOfRow:(NSIndexPath*)indexPath;
...
- (void)collectionView:(UICollectionView *)collectionView
                layout:(UICollectionViewFlowLayout*)collectionViewLayout
           didMoveCell:(NSIndexPath*)atIndexPath
...
/***
 ...
 纵向布局请调用 #import "BMCollectionViewVerticalLayout.h"
 横向布局请调用 #import "BMCollectionViewHorzontalLayout.h"
 ***/
@interface BMCollectionViewBaseFlowLayout : UICollectionViewFlowLayout

@property (nonatomic, weak) id<BMCollectionViewBaseFlowLayoutDelegate> delegate;
...
@property (nonatomic, assign) BMLayoutType layoutType;   //指定layout的类型，也可以在代理里设置

@property (nonatomic, assign) NSInteger columnCount;     //指定列数

@property (nonatomic, assign) BMColumnSortType columnSortType;   // 瀑布流列排序的方式
